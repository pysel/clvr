...
impl From<ScheduleRequest> for ScheduledTrade {
    fn from(request: ScheduleRequest) -> Self {
        ...
        let swap_params_api: ExactInputSingleParamsIntermediate = serde_json::from_str(&request.swap_params).unwrap();
        let swap_params: ExactInputSingleParams = swap_params_api.into();
